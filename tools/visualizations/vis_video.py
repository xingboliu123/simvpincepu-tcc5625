...
def main():
    ...
    for method in method_list:
        try:
            predicts_dict[method] = np.load(os.path.join(base_dir, method, 'saved_best/preds.npy'))
    ...
    for method in method_list:
        inputs = np.load(os.path.join(base_dir, method_list[0], 'saved_best/inputs.npy'))
        trues = np.load(os.path.join(base_dir, method_list[0], 'saved_best/trues.npy'))
        if 'weather' in args.dataname:
