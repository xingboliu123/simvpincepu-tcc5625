...
class PhysicsChannelMixer(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.mix_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1, groups=1)
        
       
        nn.init.dirac_(self.mix_conv.weight) 
    ...
    def forward(self, x):
        ...
        weights = self.channel_excitation(context)
        
      
        x_weighted = x * (1 + weights * 0.1)
        
        # 通道混合
        out = self.mix_conv(x_weighted)
        return out
...
class DilatedInception(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_sizes=[3, 3, 3, 3], dilations=[1, 2, 4, 8], groups=4):
        super().__init__()
        
        self.num_branches = len(kernel_sizes)
        d = C_out // self.num_branches
        self.branch_out_channels = [d] * (self.num_branches - 1) + [C_out - d * (self.num_branches - 1)]
        
        self.branches = nn.ModuleList()
        # 线性适配器 (用于自下而上)
        self.adapters_up = nn.ModuleList()
        
        for i, (k, dil) in enumerate(zip(kernel_sizes, dilations)):
            pad = (k - 1) * dil // 2
            out_ch = self.branch_out_channels[i]
            
            # 主分支卷积
            self.branches.append(nn.Sequential(
                nn.Conv2d(C_in, out_ch, kernel_size=k, padding=pad, dilation=dil, groups=groups, bias=False),
                nn.GroupNorm(min(out_ch, 8), out_ch), 
                nn.LeakyReLU(0.1, inplace=True)
            ))
            
            # 自下而上的适配器 (d_small -> d_large)
            if i > 0:
                prev_out_ch = self.branch_out_channels[i-1]
                self.adapters_up.append(nn.Sequential(
                    nn.Conv2d(prev_out_ch, C_in, kernel_size=1, bias=False),
                    nn.GroupNorm(min(C_in, 8), C_in) # 纯线性，无激活
                ))

        # 最终融合
        self.fusion = nn.Conv2d(C_out, C_out, kernel_size=1)
    ...
    def forward(self, x):
        ...
        for i, branch in enumerate(self.branches):
            ...
            if last_out is None:
                ...
                residual = self.adapters_up[i-1](last_out)
                branch_in = x + residual
            
            current_out = branch(branch_in)
            raw_outs.append(current_out)
            last_out = current_out

    
        
        refined_outs = [None] * self.num_branches
       
        refined_outs[-1] = raw_outs[-1]
        
     
        for i in range(self.num_branches - 2, -1, -1):
           
            current_feat = raw_outs[i]
           
            higher_feat = refined_outs[i+1]
            
           
            if higher_feat.shape[1] != current_feat.shape[1]:
              
                min_c = min(higher_feat.shape[1], current_feat.shape[1])
                
                refined_feat = current_feat.clone()
                refined_feat[:, :min_c, :, :] = refined_feat[:, :min_c, :, :] + higher_feat[:, :min_c, :, :]
            else:
              
                refined_feat = current_feat + higher_feat
                
            refined_outs[i] = refined_feat
            
      
        out = torch.cat(refined_outs, dim=1)
        out = self.fusion(out)
...
class gInception_ST(nn.Module):
    ...
    def __init__(self, C_in, C_hid, C_out, incep_ker=[3, 3, 3, 3], groups=8):        
        super().__init__()
        ...
        self.phy_mixer = PhysicsChannelMixer(C_hid)

    
        self.dilated_inception = DilatedInception(
            C_hid, C_out, 
